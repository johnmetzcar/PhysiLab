...
PhysiCell_file = "base_time_and_space_model_file_multiple_interventions.xml"
...
rel_output_dir = 'leukemia_time_space_model_files/'

rel_simulation_output_dir = 'leukemia_time_space_output/'
...
def createXML(intervention, substrateNames, numInterventions, eliminationID, replicateID):
    ...
    PK_parameter_set = variable1.find("PK")
    elimination_rate = PK_parameter_set.find("elimination_rate")
    if eliminationID == "1":
        elimination_rate.text = "0.01155" # 1 hour above 0.5 - current threshold
    elif eliminationID == "2":
        elimination_rate.text = "0.001925" # 6 hours above 0.5 - current threshold 
    else:
        elimination_rate.text = "0.0009627" # 12 hours above 0.5 - current threshold

    if numInterventions > 1:
        variable2.attrib = {"name": substrateNames[1], "units": "dimensionless", "ID": "0"}
        PK_parameter_set = variable2.find("PK")
        elimination_rate = PK_parameter_set.find("elimination_rate")
        if eliminationID == "1":
            elimination_rate.text = "0.01155" # 1 hour above 0.5 - current threshold
        elif eliminationID == "2":
    ...
    if numInterventions == 3:
        ...
        PK_parameter_set = variable3.find("PK")
        elimination_rate = PK_parameter_set.find("elimination_rate")
        if eliminationID == "1":
            elimination_rate.text = "0.01155" # 1 hour above 0.5 - current threshold
        elif eliminationID == "2":
    ...
    modelID = eliminationID + replicateID
...
for intervention in ibmfaDict:
    ...
    for d in range(3): # makes 3 decay rates
        ...
        decay = str(d + 1)
        #     replicate = str(r + 1)
        createXML(interventionName, substrateNames, numInterventions, decay, "1") # only one replicate for now
    # createXML(interventionName, substrateNames, numInterventions, "1", "1") # makes 1 replicate with one decay/elmination rate
...
for i in range(len(edgetic)):
    ...
    for d in range(3):
        for r in range(3):
            decay = str(d + 1)
    #         replicate = str(r + 1)
            createXML(fileName, [newName], 1, decay, "1")
